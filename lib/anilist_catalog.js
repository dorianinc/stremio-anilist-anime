...
/**
 * Airing This Season catalog (AniList)
 */
async function airingThisSeasonEntries({ offset = 0, limit = PAGE_SIZE } = {}) {
  ...
  try {
    ...
    console.log("AniList airingThisSeason: data received");

    const items = data?.Page?.media || [];
    const metas = await Promise.all(items.map(toCatalogMeta));

    console.log("AniList airingThisSeason: finished");
    return metas;
  } catch (e) {
    console.log("AniList airingThisSeason: error");
    throw e;
  }
}
