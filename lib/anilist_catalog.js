...
function toCatalogMeta(media) {
  ...
  return {
    id: `anilist:${media.id}`,
    kitsu_id: `kitsu:${media.id}`,
    type: 'anime', // keep 'anime' to match existing Kitsu add-on catalogs
  ...
  query ($page: Int, $perPage: Int) {
    Page(page: $page, perPage: $perPage) {
      media(
        ...
        id
        idMal
        format                # -> animeType (TV, TV_SHORT, ONA, OVA, SPECIAL, MOVIE)
        title { romaji english native }
        synonyms              # -> aliases (plus titles)
        description(asHtml: false)
        season                # -> optional for pretty releaseInfo like "Spring 2025"
        seasonYear            # -> releaseInfo (primary)
        coverImage { medium large extraLarge }  # -> poster
        bannerImage                              # -> background
        averageScore          # -> imdbRating (your choice to keep 0–100)
        duration              # -> runtime (per-ep minutes; document your semantics)
        episodes
        trailer { id site thumbnail }            # -> trailers
        siteUrl                                   # -> links (AniList page)
        nextAiringEpisode { episode airingAt timeUntilAiring }
        externalLinks { site url }               # -> links (merge)
      }
    }
  }
`;

  // seasonYear === released Year
