...
async function toStremioCatalogMeta(animeData) {
  console.log("🖥️ ~ animeData: ", animeData)
...
async function _toStremioMeta(animeData) {
    ...
    if (animeData.episodes.data?.length) {
      ...
      videos = animeData.episodes.data
          .map((ep, index, self) => ({
            ...
            // Already prefers English; leaving as-is
  ...
  // ---- CHANGED: build aliases and prefer English for display name ----
  const titlesArray = [
    animeData.titles?.en_us,
    animeData.titles?.en,
    animeData.titles?.en_jp,     // romaji
    animeData.titles?.ja_jp,     // native
  ...
  // Prefer English -> romaji -> native -> canonical (as last resort)
  const displayName =
    animeData.titles?.en_us ||
    animeData.titles?.en ||
  ...
  return cleanOutputObject({
    ...
    name: displayName,                        // <-- English preferred here
    slug: animeData.slug,
    aliases: titles,                          // keep all variants for search
    genres: genres || [],
