...
function cacheWrap(key, method, options) {
  // console.log("===> in cacheWrap function in cache.js")
  if (NO_CACHE || !cache) {
    return method();
  }
...
function cacheWrapCatalog(id, method) {
  // console.log("===> in cacheWrapCatalog function in cache.js")
...
async function cacheWrapIdMapping(id, method) {
  console.log("===========> where in cacheWrapIdMapping <===========");
  console.log("🖥️ ~ id: ", id)
  console.log("🖥️ ~ method: ", await method())
  const potato = await cacheWrap(`${ID_MAPPING_KEY_PREFIX}:${id}`, method, { ttl: IMAGES_TTL });
