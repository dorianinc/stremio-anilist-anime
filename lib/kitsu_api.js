...
const {
  ...
  integer,
  boolean,
} = require("parse-torrent-title/src/transformers");
...
async function animeEntries(options = {}) {
  console.log("===> in animeEntries in kitsu_api.js");
  console.log("🖥️ ~ options in animeEntries: ", options);
  if (options.trending) {
  ...
  console.log("🖥️ ~ params in animeEntries: ", params);
...
async function search(searchTerm) {
  ...
  const isEntryOvaOrSpecial = (entry) =>
    ["special", "OVA"].includes(entry.animeType);
  const isTvShow = (a, b) =>
    a.sort.title.toLowerCase().includes(b.sort.title.toLowerCase()) &&
    a.sort.tv;
  const compareSeason = (entry) => {
    if (parsedSeason === undefined)
      return parsedTitle.endsWith(`${entry.sort.season}`)
        ? -1
        : entry.sort.season || 0;
    if (entry.type !== "series") return 0;
    const entrySeason = entry.sort.season === undefined ? 1 : entry.sort.season;
    return entrySeason === parsedSeason ||
      entry.sort.title.endsWith(`${parsedSeason}`)
      ? -1
      : 0;
  };
  const compareType = (aEntry, bEntry) => {
    ...
    if (
      !isOvaOrSpecial &&
      aEntry.animeType === "special" &&
      aEntry.animeType !== bEntry.animeType
    )
      return 1;
    if (
      !isOvaOrSpecial &&
      bEntry.animeType === "special" &&
      aEntry.animeType !== bEntry.animeType
    )
      return -1;
    if (
      aEntry.sort.distance === bEntry.sort.distance &&
      isEntryOvaOrSpecial(aEntry)
    )
      return 1;
    if (
      aEntry.sort.distance === bEntry.sort.distance &&
      isEntryOvaOrSpecial(bEntry)
    )
      return -1;
  ...
  return _getCatalogEntries("anime", query)
    .then((entries) =>
      entries.map((entry) => {
        const parsedTitles = [entry.name]
          .concat(entry.aliases)
          .map((title) => parser.parse(title));
        const distances = parsedTitles.map((parsed) =>
          distance(parsedTitle, escapeTitle(parsed.title))
        );
        ...
        const maxSeason =
          parsedTitles.map((v) => v.season).find((v) => v) ||
    ...
    .then((entries) =>
      entries.sort((a, b) => {
        ...
        const hasExactMatch =
          (a.sort.distance === 1 || b.sort.distance === 1) &&
          distanceSort !== 0;
...
async function list(kitsuIdsRaw) {
  console.log("🖥️ ~ kitsuIdsRaw: ", kitsuIdsRaw);
...
async function _list(params, offset = 0) {
  return kitsuApi.get("anime", { params }).then((response) => {
    if (response.links.next) {
      const nextOffset = offset + response.data.length;
      return _list(
        { ...params, page: { ...params.page, offset: nextOffset } },
        nextOffset
...
async function animeData(kitsuId) {
  console.log("🖥️ ~ kitsuId in kitsu_api: ", kitsuId)
  const params = { include: "genres,episodes,mediaRelationships.destination" };
  const res = await kitsuApi
    .get(`anime/${kitsuId}`, { params })
    .then((response) => {
      // console.log("🖥️ ~ response.data: ", response.data)
      return toStremioEntryMeta(response.data)
    })
    .catch((error) =>
...
async function _getCatalogEntries(endpoint, params = {}) {
  ...
  console.log("in _getCatlogEntries in kitsu_api.js");
  ...
  // params.filter.season = "winter";
  // params.filter.seasonYear = 2022
  params.page = params.page || {};
  params.page.limit = PAGE_SIZE;
  console.log(
    `===> params for ${endpoint} before being sent to kitsu api: `,
    params
  );
  const results = await kitsuApi
    .get(endpoint, { params })
    .then((response) =>
      Promise.all(response.data.map((result) => {
        // console.log("👺 single result ====>", result)
        return toStremioCatalogMeta(result)
      }))
    );
...
function initSeasonParser() {
  ...
  addDefaults(parser);
  parser.handlers = parser.handlers.filter((handler) =>
    ["seasons", "season"].includes(handler.handlerName)
  );
