...
async function animeEntries(options = {}) {
  console.log("animeEntries start");

  if (options.trending) {
    console.log("animeEntries trending path");
    return trendingEntries(options.limit);
  }

  console.log("animeEntries normal path");
  const params = { filter: {}, page: {} };
  if (options.offset) params.page.offset = options.offset;
  if (options.genre) params.filter.genres = options.genre;
...
async function trendingEntries(limit = 50) {
  console.log("trendingEntries start");
...
async function search(searchTerm) {
  ...
  return _getCatalogEntries('anime', query)
    .then((entries) => {
      console.log("search got entries");
    ...
    .then((entries) => {
      console.log("search sorting entries");
      return entries.sort((a, b) => {
        ...
        const seasonSort = aSeasonSort - bSeasonSort;
        const typeSort = aTypeSort - bTypeSort;
    ...
    .then((entries) => {
      console.log("search cleaned up entries");
      return entries.map((entry) => {
        delete entry.sort;
        return entry;
      });
...
async function list(kitsuIdsRaw) {
  ...
  const params = {
    include: 'genres',
    filter: { id: kitsuIds.join(',') },
    page: { limit: PAGE_SIZE }
  };

  return _list(params)
    .then(responses => {
      console.log("list got responses");
      return responses.reduce((a, b) => a.concat(b.data), []);
    })
    .then(metas => {
      console.log("list building metas");
...
async function _list(params, offset = 0) {
  console.log("_list call");
  return kitsuApi.get('anime', { params })
    .then(response => {
      if (response.links.next) {
        console.log("_list has next page");
...
async function _getCatalogEntries(endpoint, params = {}) {
  ...
  return kitsuApi.get(endpoint, { params })
    .then((response) => {
      console.log("_getCatalogEntries got response");
      return Promise.all(response.data.map((result) => toStremioCatalogMeta(result)));
    });
}
